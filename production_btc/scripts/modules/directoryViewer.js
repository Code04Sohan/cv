...
window.DirectoryViewerModule = (function () {
    ...
    function renderTableBody() {
        ...
        _filteredData.forEach((row) => {
            ...
            // Resolve photo for table avatar
            const resolvedPhoto = resolveDriveUrl(row.STUDENT_PHOTO_URL);
            const avatarSrc = resolvedPhoto || SVG_FALLBACK_AVATAR;

            html += `
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors group">
                    <td class="px-6 py-4">
                        <img src="${avatarSrc}" crossorigin="anonymous"
                            class="w-10 h-10 rounded-full object-cover border border-slate-200 dark:border-slate-700 bg-slate-50"
                            alt="Avatar"
                            onerror="this.src='${SVG_FALLBACK_AVATAR}'">
                    </td>
                    <td class="px-6 py-4">
     ...
     * Async: resolves photo and signature from Drive before rendering.
     */
    async function viewRecord(index) {
        const data = _directoryData[index];
        if (!data) return;

        // --- Async: Resolve Drive images to Base64 ---
        let photoBase64 = null;
        let signBase64 = null;
        try {
            [photoBase64, signBase64] = await Promise.all([
                getBase64FromDriveUrl(data.STUDENT_PHOTO_URL),
                getBase64FromDriveUrl(data.STUDENT_SIGNATURE_URL)
            ]);
        } catch (e) {
            console.warn("Drive image resolution silently failed:", e);
        }
        ...
        const photoHtml = photoBase64
            ? `<img src="${photoBase64}" style="width:100px;height:120px;" class="object-cover rounded-xl shadow-lg border-2 border-white dark:border-slate-700 shrink-0">`
        ...
        const signHtml = signBase64
            ? `<img src="${signBase64}" class="h-14 max-w-[180px] w-auto object-contain">`
            : `<span class="text-xs font-semibold text-slate-400 italic">No signature on file</span>`;
                ...
                <div class="bg-slate-50 dark:bg-slate-900 w-full max-w-5xl max-h-[92vh] rounded-2xl shadow-2xl flex flex-col border border-slate-200 dark:border-slate-800 overflow-hidden">
                    ...
                    <div class="overflow-y-auto custom-scrollbar flex-1 p-6 space-y-5">
                        ...
                        ${section('Academic Profile', 'bg-brand-100 dark:bg-brand-900/40 text-brand-600',
                            ...
                            field('Date of Admission', data.DATE_OF_ADMISSION)
                        )}
                        ...
                        ${section('Administrative & Financial', 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-600',
                            ...
                            `<div class="col-span-2 md:col-span-4 flex flex-col gap-0.5">
                                ...
                                <span class="text-sm font-semibold text-slate-800 dark:text-slate-100">${data.TIMESTAMP || '—'}</span>
                            </div>`
                        )}
                        ...
                        <div class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                            <div class="flex items-center gap-2.5 px-5 py-3.5 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/60">
                                ...
                                <h3 class="text-xs font-black text-slate-600 dark:text-slate-300 uppercase tracking-wider">Verified Signature</h3>
                            </div>
                            <div class="p-5 flex items-center gap-8">
                                <div class="bg-slate-50 dark:bg-slate-800 border border-dashed border-slate-300 dark:border-slate-600 rounded-xl px-8 py-5 flex items-center justify-center min-h-[80px] min-w-[200px]">
                                    ${signHtml}
                                </div>
                                <div class="flex flex-col gap-1">
                                    <span class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Declaration Date</span>
                                    <span class="text-sm font-bold text-slate-700 dark:text-slate-200">${data.DATE_OF_ADMISSION || '—'}</span>
                                    <span class="text-[10px] text-slate-400 mt-2">Candidate has agreed to all stated</span>
                                    <span class="text-[10px] text-slate-400">declarations and terms at the time of admission.</span>
                                </div>
                            </div>
                        </div>
    ...
    function editRecord(index) {
        ...
        // Resolve photo/signature for preview thumbnails (sync fallback)
        const photoSrc = resolveDriveUrl(data.STUDENT_PHOTO_URL) || SVG_FALLBACK_AVATAR;
        const signSrc = resolveDriveUrl(data.STUDENT_SIGNATURE_URL) || "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 24 24' fill='%23cbd5e1'><rect width='24' height='24' fill='none'/></svg>";
                        ...
                        <div>
                            ...
                            <div class="flex gap-8 items-start">
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <img id="edit_preview_photo" src="${photoSrc}" crossorigin="anonymous" class="w-24 h-28 object-cover rounded-xl border border-slate-200 bg-slate-50" onerror="this.src='${SVG_FALLBACK_AVATAR}'">
                                ...
                                <div class="flex flex-col items-center gap-3">
                                    ...
                                    <img id="edit_preview_sign" src="${signSrc}" crossorigin="anonymous" class="h-16 w-32 object-contain rounded-xl border border-slate-200 bg-slate-50 px-2">
